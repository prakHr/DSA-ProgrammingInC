...
int can_we_transfer(int N,int dir_flag,int which,int part[][3],int folder[]);
...
int main(){
	...
	int flag=can_we_transfer(N,0,N-1,part,folder);
	printf("%d\n",flag);
...
void flow_heat(int N,int part[][3],int folder[]){
	//be caution call next fn at the endpoints.
	//also dont shrink when only one obejct in partition
	do{
		int swap_flag;
		for(int i=0;i<N;i++){
...
int can_we_transfer(int N,int dir_flag,int which,int part[][3],int folder[]){
	...
	if(dir_flag==0){
		...
		for(int i=which;i>0;i--){
			...
			else if(i==which-1 && (c_temp+heat_in-heat_out>max_temp)){
				ok_flag=0;
				break;
			}
		...
		for(int i=which;i<N-1;i++){//moving to last partitons 
			...
			if(i==N-2 && (c_temp+heat_in)>max_temp){
				...
				//atleast ek bhi kam wala mil jaye
				break;
			}
		}
